...
function initNav(){
    initNavHtml();
    $.ajax({
        url:"http://127.0.0.1:8088/fruit-web/src/js/nav.json",
        type:"post",
        dataType:'json',
        async:false,
...
/*            var $li=$("<li id="+treeData[i].id+" class="+getIco(treeData[i].name)+">"+
            ...
            var $li=$("<li id="+treeData[i].id+"><a><i class='fa fa-home'></i> "+ treeData[i].name+
    ...
    for(var i=0;i<treeData.length;i++){
        if(treeData[i].pId!=0){
            ...
            if($ul.length==0){ //表示当前还没有ul
                //创建ul
                $ul="<ul class='nav child_menu'></ul>";
                //把ul加到pli中
                $pli.append($ul);
            }

            var $sonli=$("<li><a href=\""+treeData[i].url+"\">"+treeData[i].name+"</a></li>");
            $ul=$pli.children("ul");
            $ul.append($sonli);
        }
    }
